...
def validate_input(job_input):
    ...
    # Validate 'audios' in input, if provided. The upstream worker only stages
    # images. FusionInteract lip-sync jobs need audio files available to
    # ComfyUI's LoadAudio node by filename.
    audios = job_input.get("audios")
    if audios is not None:
        if not isinstance(audios, list) or not all(
            "name" in audio and "audio" in audio for audio in audios
        ):
            return (
                None,
...
def _safe_input_filename(name, fallback_extension=".mp3"):
    """Return a filename safe for direct placement in ComfyUI's input folder."""
    basename = os.path.basename(str(name or ""))
    basename = re.sub(r"[^A-Za-z0-9._-]", "_", basename).strip("._")
    if not basename:
        basename = f"fusioninteract-audio-{uuid.uuid4().hex}{fallback_extension}"
    return basename[:180]


def upload_audios(audios):
    """
    Stage base64 encoded audio files into ComfyUI's input directory.

    The ComfyUI LoadAudio node validates by looking for a file in the input
    folder, not through /upload/image. RunPod's base worker does not know about
    audio uploads, so this worker adds that small missing bridge.
    """
    if not audios:
    ...
    print(f"worker-comfyui - Uploading {len(audios)} audio file(s)...")

    for audio in audios:
        try:
            name = _safe_input_filename(audio["name"])
            audio_data_uri = audio["audio"]

            if "," in audio_data_uri:
                base64_data = audio_data_uri.split(",", 1)[1]
            else:
                base64_data = audio_data_uri
            ...
            responses.append(f"Successfully uploaded {name}")
            print(
                f"worker-comfyui - Successfully staged audio {name} ({len(blob)} bytes)"
            )

        except base64.binascii.Error as e:
            error_msg = f"Error decoding base64 for {audio.get('name', 'unknown')}: {e}"
            print(f"worker-comfyui - {error_msg}")
            upload_errors.append(error_msg)
        except Exception as e:
            error_msg = (
                f"Unexpected error uploading {audio.get('name', 'unknown')}: {e}"
            )
            print(f"worker-comfyui - {error_msg}")
            upload_errors.append(error_msg)

    if upload_errors:
        print(f"worker-comfyui - audio upload finished with errors")
        return {
            "status": "error",
    ...
    print(f"worker-comfyui - audio upload complete")
    return {
        "status": "success",
...
def handler(job):
    ...
    # Stage input audio files if they exist
    if input_audios:
        upload_result = upload_audios(input_audios)
        if upload_result["status"] == "error":
            return {
                "error": "Failed to upload one or more input audios",
                "details": upload_result["details"],
            }
